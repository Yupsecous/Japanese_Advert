...
function RefinementList({ items }: { items: string[] }) {
  const t = useT();
  if (items.length === 0) {
    return <p className="text-sm text-neutral-500">{t('notes.noRefinements')}</p>;
  }
  return (
    <ul className="space-y-1.5 text-sm text-neutral-800">
      {items.map((s, i) => (
        <li key={i} className="flex gap-2">
          ...
          <span>
            <span className="text-neutral-500">{t('notes.pushedFor')} </span>
...
export function DirectorsNotes() {
  ...
  return (
    <section className="space-y-6 text-sm">
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('notes.brief')}</h3>
        <dl className="mt-2 grid grid-cols-[120px_1fr] gap-y-1.5 text-sm">
          <dt className="text-neutral-500">{t('notes.product')}</dt>
          <dd className="text-neutral-900">{d.brief.productName}</dd>
          <dt className="text-neutral-500">{t('notes.audience')}</dt>
          <dd className="text-neutral-900">{d.brief.targetAudience}</dd>
          <dt className="text-neutral-500">{t('notes.angle')}</dt>
      ...
      {d.copy && (
        <div>
          ...
          <p className="mt-2 text-neutral-800">
            {t('notes.approvedVariant')} <span className="font-mono">#{d.copy.approvedIndex + 1}</span>:{' '}
            <span className="font-medium text-neutral-900">&ldquo;{d.copy.headline}&rdquo;</span>
          </p>
          <div className="mt-3">
            <p className="text-xs font-medium text-neutral-600">{t('notes.refinements')}</p>
      ...
      {d.image && (
        <div>
          ...
          <p className="mt-2 text-neutral-800">
            {t('notes.approvedVariant')} <span className="font-mono">#{d.image.approvedIndex + 1}</span>.
          </p>
          <div className="mt-3">
            <p className="text-xs font-medium text-neutral-600">{t('notes.refinements')}</p>
            <div className="mt-1.5">
          ...
          {d.image.appliedCritiques.length > 0 && (
            <div className="mt-3">
              <p className="text-xs font-medium text-neutral-600">{t('notes.critiquesApplied')}</p>
      ...
      {d.script && (
        <div>
          ...
          <p className="mt-2 text-neutral-800">
            {t('notes.approvedVariant')} <span className="font-mono">#{d.script.approvedIndex + 1}</span>:{' '}
            <span className="text-neutral-700">&ldquo;{d.script.scriptSnippet}&rdquo;</span>
          </p>
          {d.script.voice && (
            <p className="mt-1 text-neutral-800">
              {t('final.voice')}: <span className="font-medium text-neutral-900">{d.script.voice.name}</span>{' '}
      ...
      {d.audio && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('step.audio')}</h3>
          <p className="mt-2 text-neutral-800">
            {t('notes.audioAttempt')} <span className="font-mono">#{d.audio.attempt}</span>
            {d.audio.regenerateCount > 0 && (
              <span className="text-neutral-500">
                {' '}
                {t('notes.afterRegen', {
                  n: d.audio.regenerateCount,
                  s: d.audio.regenerateCount === 1 ? '' : 's',
                })}
              </span>
            )}
            .
