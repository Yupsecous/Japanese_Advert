...
- **Design step** — single-file React + Tailwind landing-page component
  generated by claude-opus-4-7. Rationale + scrollable code box + copy-
  to-clipboard. Refine box accepts plain English.
- **i18n** — 6 locales (EN, 日本語, PT, ES, FR, DE). EN + JA have full
  coverage; the other 4 cover headings + buttons (EN fallback for the
  rest). Pill picker in the auth + brief headers.

All six steps are now port-complete end-to-end, in six languages.
Audience Console and Platform exports are the next big features.
